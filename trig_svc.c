...
static void
trig_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		double seno_1_arg;
		double coseno_1_arg;
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, TRIG, VTRIG, trig_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (TRIG, VTRIG, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, TRIG, VTRIG, trig_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (TRIG, VTRIG, tcp).");
		exit(1);
	}
